...
let transform_rule (view:rterm) (cnt:colnamtab) (rule:stt) lst=
  ...
  | Source _        -> rule::lst
  | View _          -> rule::lst
  | Source_schema _  -> invalid_arg "function transform_rul called with a source_schema"
  | Target_schema _  -> invalid_arg "function transform_rul called with a target_schema"
  | Rule(h, body) ->
...
let remove_constraint_of_view (log:bool) prog =
  ...
  if Hashtbl.mem idb (symtkey_of_rterm get_empty_pred) then
    ...
    if log then (
      print_endline "_____constraints not involing view_______"; (* tentative, Jumpei Tanaka *)
...
let keep_only_constraint_of_view (log:bool) prog =
  ...
  if Hashtbl.mem idb (symtkey_of_rterm get_empty_pred) then
    ...
    if log then (
      print_endline "_____constraints involing view_______";  (* tentative, jumpei tanaka *)
...
let extract_projection stt ind= match stt with
    ...
    | Source _        -> invalid_arg "function extract_projection called with a source"
    | View _          -> invalid_arg "function extract_projection called with a view"
    | Source_schema _ -> invalid_arg "function extract_projection called with a source schema"
    | Target_schema _ -> invalid_arg "function extract_projection called with a target schema"
    | Rule(h, b) ->
