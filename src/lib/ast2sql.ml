...
let sql_of_vterm (vt:vartab) (eqt:eqtab) (expr:vterm)  =
  ...
  (*          | Skolemf _ -> invalid_arg "function sql_of_vterm called with a skolemf"   *)
        in sql_of 0 expr;;
...
(*    if (debug) then print_endline ("==> generating SQL query of datalog program of query "^ string_of_stt (get_query prog)) else (); *)
    (* tebtative modificaiton by Jumpei Tanaka*)
    if (debug) then print_endline ("==> generating SQL query of datalog program of query "^ string_of_stt (get_query prog)) else ();
...
let view_constraint_sql_of_stt (dbschema:string) (debug:bool) prog =
    ...
    if Hashtbl.mem idb (symtkey_of_rterm get_empty_pred) then
        ...
        if Hashtbl.mem idb (symtkey_of_rterm get_empty_pred) then
            ...
            let prog4 = (insert_stt (Query get_empty_pred) prog3) in
...
let non_view_constraint_sql_of_stt (dbschema:string) (debug:bool) prog =
...
let non_rec_unfold_sql_of_update (dbschema:string) (debug:bool) prog (delta:rterm)  =
    (*
    *)
    let prog2 =
        prog in
...
let unfold_delta_sql_stt (dbschema:string) (debug:bool) prog =
        ...
        (*Return the desired SQL*)
        let update_sql_lst = List.map (non_rec_unfold_sql_of_update dbschema debug prog2 ) delta_rt_lst in
...
let unfold_delta_trigger_stt (dbschema:string) (debug:bool) prog =
    let view_rt = get_schema_rterm (get_view prog) in
    ...
    let (vardec, delta_sql_stt) = unfold_delta_sql_stt dbschema debug prog in
    let trigger_pgsql =
...
AS $$
  ...
  BEGIN
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = '"^view_name^"_delta_action_flag') THEN
        ...
        IF EXISTS (" ^ view_constraint_sql_of_stt dbschema debug prog^" )
        THEN
          RAISE check_violation USING MESSAGE = 'Invalid view update: constraints on the view are violated';
        END IF;
        IF EXISTS (" ^ non_view_constraint_sql_of_stt dbschema debug prog^" )
        THEN
