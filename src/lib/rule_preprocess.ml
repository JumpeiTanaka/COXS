...
let extract_rterm_constants ?(pos = 0) rt = match rt with
    ...
    | _ ->  invalid_arg "function extract_rterm_constants called"
...
let extract_rule_constants = function
    ...
    | Source_schema _ -> invalid_arg "function extract_rule_constants called with a source schema"
    | Target_schema _ -> invalid_arg "function extract_rule_constants called with a target schema"
;;
...
let anonvar2namedvar = function
    ...
    | Source _        -> invalid_arg "function anonvar2namedvar called with a source"
    | View _          -> invalid_arg "function anonvar2namedvar called with a view"
    | Source_schema _ -> invalid_arg "function anonvar2namedvar called with a source schema"
    | Target_schema _ -> invalid_arg "function anonvar2namedvar called with a target schema"
    | Rule(h, b) ->
        ...
        let a2n_term t (lst, ind)  = match t with
            | Rel rt -> (match rt with
                        ...
                        | _ -> invalid_arg "function anonvar2namedvar"
                        )
            | Not rt -> (t::lst, ind)
