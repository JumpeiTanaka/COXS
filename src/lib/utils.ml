...
let print_symtable (st:symtable) =
  ...
  let print_lst key lst = Printf.printf "(%s, %d) ==>  \n" (fst key) (snd key); List.iter print_el lst in
...
(* @author: Jumpei Tanaka *)
let extract_schema = function
  | Prog stt_lst ->
    let schema:symtable = Hashtbl.create (2 * (List.length stt_lst)) in
    let in_stt t = match t with
      | Rule _          -> ()
      | Constraint _    -> ()
      | Pk _            -> ()
      ...
      | View _
      | Source_schema _
      | Target_schema _  -> symt_insert schema (Rule (get_schema_rterm t,[])) in
    List.iter in_stt stt_lst;
    schema
...
let variablize_rterm(rt:rterm) = match rt with
  ...
  | _ -> invalid_arg "error: Utils.variablize_rterm" (* by Jumpei Tanaka, due to Deltainseet_ot, Deletadelete_ot *)
;;
...
let get_temp_rterm (rt:rterm) = match rt with
  ...
  | _ -> invalid_arg "error: Utils.get_temp_rterm" (* by Jumpei Tanaka, due to Deltainseet_ot, Deletadelete_ot *)
;;
...
let rename_rterm (prefix:string) (rt:rterm) = match rt with
  ...
  | _ -> invalid_arg "error: Utils.rename_rterm" (* by Jumpei Tanaka, due to Deltainseet_ot, Deletadelete_ot *)
;;
...
let change_vars rt vs = match rt with
            ...
            | _ -> invalid_arg "error: Utils.change_vars" (* by Jumpei Tanaka, due to Deltainseet_ot, Deletadelete_ot *)
...
let get_source_rterms e = match e with
  | Prog sttl ->
    let add_source (rtset:rtermset) stt= match stt with
      ...
      | Source_schema _ -> RtermSet.add (get_schema_rterm stt) rtset
      | _ -> rtset
    in
...
(** get all target predicates *)
let get_target_rterms e = match e with
  | Prog sttl ->
    let add_target (rtset:rtermset) stt= match stt with
      | Target_schema _ -> RtermSet.add (get_schema_rterm stt) rtset
      | _ -> rtset
    in
    let target_lst: rterm list = RtermSet.elements (List.fold_left add_target RtermSet.empty sttl) in
    target_lst
;;

(* all predicates in rule head *)
let get_rulehead_rterms prog =

  let rec _1_get_rule_rterms prog = match prog with
    | Prog sttl -> _2_get_rule_rterms sttl

    and _2_get_rule_rterms sttl = match sttl with
      | [] -> []
      | hd::rest -> (_3_get_rule_rterm hd) @ (_2_get_rule_rterms rest)

    and _3_get_rule_rterm hd = match hd with
      | Rule (Pred(name, varlst), _) -> [Pred(name, varlst)]
      | _ -> []
  in

  let result = _1_get_rule_rterms prog in
result
;;
...
let is_delta_or_empty rt = match rt with
  ...
  | Deltadelete _ -> true
  | Deltainsert _ -> true
  | _ -> invalid_arg "error: Utils.is_delta_or_empty" (* by Jumpei Tanaka, due to Deltainseet_ot, Deletadelete_ot *)
...
(* Jumpei Tanaka *)
let constraint2constraint prog = match prog with
    | Prog stt_lst ->
    let trans_stt t lst= match t with
      ...
      | Pk (relname, attrlst) ->
        ...
        (List.map (fun x -> Constraint(get_empty_pred, [Rel (Pred(relname, List.map (fun t -> NamedVar t) allattrlst)); Rel (Pred(relname, List.map (fun t -> NamedVar t) allattrlst2)); Equal(Var (NamedVar x), Var (NamedVar (x^"2")))] )) nonkeyattrlst )@lst
      | Query _ -> t::lst
      | Source _ -> t::lst
      | View _ -> t::lst
      | Source_schema _ -> t::lst
      | Target_schema _ -> t::lst
      in
