...
let steps ast0 log timeout dbschema physical =
  ...
  (* ************************************************ *)
  print_endline "(* ************************************************ *)";
  print_endline "(*         Target-side Database                     *)";
  print_endline "(* ************************************************ *)";
  ...
  (* mapping between var and stype *)
  let var_stype = Preperation.get_var_stype ast in
  ...
  let pk_lst_var = Preperation.get_pk_pred_var ast0 log in
...
let t_src_ast = View_ts.derivation_source_birds ast_schemas ast_constraint_core_1 ast_constraint_pk init_pred_lst head_preds_target_1 target_pred_1 ast_schevo_1 source_rels pk_lst pk_lst_var var_stype log in
(* pk_lst and pk_lst_var shuld be _1, but not yet coded for them *)

(* exeucte birds and generate SQL *)
let all_source_rels = unique_element @@ List.fold_left  List.append [] (List.map get_source_rels_schema t_src_ast)
in
       printf "all_source_rels=> [";
       let print_el s = printf "%s; " s in
       List.iter print_el all_source_rels;
       printf "]\n";

let schema_mpg_lst = View_init.mpg_init all_source_rels log physical in
let all_schema_mpg_src_lst = unique_element @@
                          schema_mpg_lst_1 @ schema_mpg_lst in

       printf "all_source_rels=> [\n";
       let print_el s = printf "(%s, %s); " (fst s) (snd s) in
       List.iter print_el all_schema_mpg_src_lst;
       printf "\n]\n";

print_endline "run BIRDS for source schema ...";
let birds_src_lst = execute_birds t_src_ast log dbschema true in (* set true to make verificaiton by BIRDS *)
let birds_sql_src_lst = List.map snd birds_src_lst in
let birds_status_src_lst = List.filter (fun x -> match x with 0 -> false | _ -> true) (List.map fst birds_src_lst) in

let dummy =
  if (List.length birds_status_src_lst) != 0
  then begin printf ""; end
  else begin
    let e = rewrite_sql_schemas log dbschema all_schema_mpg_src_lst in
    List.iter e birds_sql_src_lst;
    end
in
...
let t_trg_ast = View_ts.derivation_target_birds
                  ast_schemas
                  ast_constraint_core_1
                  ast_constraint_pk
                  init_pred_lst
                  head_preds_target_1
                  target_pred_1
...
(* exeucte birds and generate SQL *)
let all_source_rels_trg = unique_element @@ List.fold_left  List.append [] (List.map get_source_rels_schema t_trg_ast)
in
       printf "all_source_rels_trg=> [";
       let print_el s = printf "%s; " s in
       List.iter print_el all_source_rels_trg;
       printf "]\n";

let schema_mpg_lst_trg = View_init.mpg_init all_source_rels_trg log physical in
let all_schema_mpg_trg_lst = unique_element @@
                          schema_mpg_lst_1 @ schema_mpg_lst in

       printf "all_source_rels_trg=> [\n";
       let print_el s = printf "(%s, %s); " (fst s) (snd s) in
       List.iter print_el all_schema_mpg_trg_lst;
       printf "\n]\n";

print_endline "run BIRDS for target schema ...";
let birds_trg_lst = execute_birds t_trg_ast log dbschema true in (* set true to make verificaiton by BIRDS *)
let birds_sql_trg_lst = List.map snd birds_trg_lst in
let birds_status_trg_lst = List.filter (fun x -> match x with 0 -> false | _ -> true) (List.map fst birds_trg_lst) in

let dummy =
  if (List.length birds_status_trg_lst) != 0
  then begin printf ""; end
  else begin
    let e = rewrite_sql_schemas log dbschema all_schema_mpg_trg_lst in
    List.iter e birds_sql_trg_lst;
    end
in
  ...
  (* Show final result                                                *)
  (* **************************************************************** *)
  let birds_result = birds_src_lst @ birds_trg_lst in
